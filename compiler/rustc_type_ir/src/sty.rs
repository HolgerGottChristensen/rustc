...
pub enum TyKind<I: Interner> {
    ...
    Argument(I::ArgumentDef, I::DefId),
...
impl<I: Interner> Clone for TyKind<I> {
    fn clone(&self) -> Self {
        match self {
            ...
            Argument(s, d) => Argument(s.clone(), d.clone())
        }
    }
}
    ...
    fn eq(&self, other: &TyKind<I>) -> bool {
        tykind_discriminant(self) == tykind_discriminant(other)
            && match (self, other) {
                ...
                (Argument(a_s, a_d), Argument(b_s, b_d)) => a_s == b_s && a_d == b_d,
    ...
    fn cmp(&self, other: &TyKind<I>) -> Ordering {
        tykind_discriminant(self).cmp(&tykind_discriminant(other)).then_with(|| {
            match (self, other) {
                ...
                (Argument(a_p, a_d), Argument(b_p, b_d)) => a_p.cmp(b_p).then_with(|| a_d.cmp(b_d)),
...
impl<I: Interner> fmt::Debug for TyKind<I> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Argument(v, d) => f.debug_tuple_field2_finish("Argument", v, d),
    ...
    I::AllocId: Encodable<E>,
{
    fn encode(&self, e: &mut E) {
        ...
        match self {
            ...
            Argument(v, d) => e.emit_enum_variant(disc, |e| {
                v.encode(e);
                d.encode(e);
            })
        }
