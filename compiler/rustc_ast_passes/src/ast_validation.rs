...
fn validate_generic_param_order(
    ...
    for (idx, param) in generics.iter().enumerate() {
        ...
        let (ord_kind, ident) = match kind {
            ...
            GenericParamKind::HKT(_) | GenericParamKind::Type { .. } => (ParamKindOrd::TypeOrConst, ident.to_string()),
            ...
            for param in &generics.params {
                if param.ident == potential_param.ident {
                    for bound in &param.bounds {
                        if let GenericBound::Trait(trait_ref, TraitBoundModifier::None) = bound {
                            if let [trait_segment] = &trait_ref.trait_ref.path.segments[..] {
                                let assoc = pprust::path_to_string(&Path::from_ident(
